...
app.use(body())
let port = 3000;
// http://benjaminh.promo-29.codeur.online:3000

const encoderJson = 'http://benjaminh.promo-29.codeur.online/fr2phonetics_api/encoder/model.json';
const decoderJson = 'http://benjaminh.promo-29.codeur.online/fr2phonetics_api/decoder/model.json';
let encoder_model;
let decoder_model;
...
app.post('/', async (req, res) => {
    const word = req.body.word; // récupération des variables du body
    let toTranslate = JSON.parse(word);
    let encoded_words = [];
    let decoded_data = {};
    toTranslate.forEach(element => {
        encoded_words.push(encodeWord(element));
    });
    loadModel()
    .then(() => {
        for(let i=0;i<toTranslate.length;i++) {
            decoded_data[toTranslate[i]] = translate(encoded_words[i]).slice(0, -1);
        }
...
function translate(input_sequence) {
